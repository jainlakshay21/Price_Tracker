...
def Submit():
   ...
   def checkprice():
      ...
      if convertedprice<int(DP.get()):
         global discprice
         discprice= format_currency(DP.get(), 'INR', locale='en_IN')
         send_mail()
   ...
   def send_mail():
      ...
      body=f'Your product {title.text} is selling for {price.text} {discprice}. Go and Buy now at the link below: {URL.get()}'
      msg = f"Subject: {subject}\n\n{body}".encode('utf-8')
      server.sendmail('worklakshay21@gmail.com',Email.get(),msg)
